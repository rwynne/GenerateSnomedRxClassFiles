...
public class GenerateSnomedRxClassFiles {
	...
	public void addCountsToClasses() {
		HashMap<ClassMember, Integer> classToCount = new HashMap<ClassMember, Integer>();
		for(ClassMember cm : classMemberList) {
			OWLClass cls = cm.product;
			Integer count = 0;
			for( DrugMember drugMem : drugMemberList ) {
				if( drugMem.getProductForDirect().equals(cls) ) {
					if( cm.getClassId().equals("350157004") ) {
						System.out.println("adding to " + cm.getClassName() + getRDFSLabel(drugMem.getProductForDirect()));  
					}
					count++;
				}
			}
			classToCount.put(cm, count);
		}
		
		for( ClassMember product : classToCount.keySet() ) {
			Integer countOfDrugMembers = classToCount.get(product);
			for( ClassMember listItem : classMemberList ) {
				if( listItem.getClassId().equals(product.getClassId()) && listItem.getClassType().equals(product.getClassType()) ) {
					ClassMember classToAdd = listItem;
					classToAdd.setCountDrugMembers(countOfDrugMembers);
					classMemberListWithCounts.add(classToAdd);
					break;
				}
			}
						...
						OWLClass productForIndirect = order.remove(0);
							for( OWLClass mp : mps ) {
								if( this.sct2RxIN.containsKey(Long.valueOf(getId(mp))) ) {
									in = sct2RxIN.get(Long.valueOf(getId(mp))).get(0);
									if( in != null ) {
										DrugMember drugMember = new DrugMember(_1_source, _2_relationship, productForDirect, in, mp, false, p);
										drugMemberList.add(drugMember);										
...
//		this.classTreeFile.println(root.getClassType() + "|" + "|" + root.getRootCode() + "|" + root.getRootName() + "|" +  root.getRootCode() + "||0||" + "|0"); //post-processing, populate 6 with the number of unique mps
		for(OWLClass c : this.classPathMap.keySet()) {		
			for( Path p : this.classPathMap.get(c) ) {
				...
				if( !classMemberList.contains(classMember) ) {
